...
class Step1App:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Tick Downloader (STEP1)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(row, text="Start date (JST)").grid(row=0, column=0, sticky="w")
        start_entry = ttk.Entry(row, textvariable=self.start_var, width=12, state="readonly")
        start_entry.grid(row=0, column=1, padx=6)
        ttk.Button(row, text="Pick", command=self._pick_start).grid(row=0, column=2)

        ttk.Label(row, text="End date (JST)").grid(row=1, column=0, sticky="w", pady=(6, 0))
        end_entry = ttk.Entry(row, textvariable=self.end_var, width=12, state="readonly")
        end_entry.grid(row=1, column=1, padx=6, pady=(6, 0))
        ttk.Button(row, text="Pick", command=self._pick_end).grid(row=1, column=2, pady=(6, 0))

        self.run_button = ttk.Button(frame, text="Download", command=self._start_download)
    ...
    def _start_download(self):
        if self.worker and self.worker.is_alive():
            messagebox.showinfo("Info", "Download is running.")
            return
    ...
    def _download_worker(self, start: date, end: date):
        ...
        total = len(hours)
        self.queue.put(("status", f"Downloading {total} files..."))
        ...
        for idx, dt_utc in enumerate(hours, start=1):
            ...
            if path.exists():
                if path.stat().st_size == 0:
                    ...
                    self.queue.put(("log", f"[{idx}/{total}] skip {path}"))
                    continue
            ...
            try:
                ...
                if path.stat().st_size == 0:
                    ...
                    self.queue.put(("log", f"[{idx}/{total}] 0バイト {url}"))
                    continue
                self.queue.put(("log", f"[{idx}/{total}] ok   {path}"))
            except HTTPError as e:
                self.queue.put(("log", f"[{idx}/{total}] http {e.code} {url}"))
            except URLError as e:
                self.queue.put(("log", f"[{idx}/{total}] url error {e.reason}"))
            except Exception as e:
                self.queue.put(("log", f"[{idx}/{total}] error {e}"))
